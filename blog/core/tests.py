...
class UserProfileTestCase (TestCase):
    ...
    def test_userprofile_str(self):
        self.assertEqual(f"{self.first_userprofile.user.username} {self.first_userprofile.profession}", str(
            self.first_userprofile))
...
class BlogFormTest(TestCase):
    ...
    def test_form_labels(self):
        self.assertTrue(
            self.form.fields["title"].label is None or self.form.fields["title"].label == "title")
        self.assertTrue(
            self.form.fields["description"].label is None or self.form.fields["description"].label == "description")
        self.assertTrue(
            self.form.fields["blog_category"].label is None or self.form.fields["blog_category"].label == "blog_categorye")
        self.assertTrue(
            self.form.fields["image"].label is None or self.form.fields["image"].label == "image")
...
class BlogTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.first_blog = Blog.objects.create(
            ...
            blog_category="1",
            user=self.user,
            image=SimpleUploadedFile(name='baby.jpg', content=open(
                '/home/almastasoyan/Desktop/Courses/women_blogapp/blog/media/Media/baby.jpg', 'rb').read())
        )
